...
```{r}
# Produced by scripts (1), (2), (3) after running their simulation chunks.
# Save the `combine_pop1` / `combine_pop` outputs to these files before running (4).
...
```{r}
raw_IL2   <- read.csv("") # Not provided
raw_IFNg  <- read.csv("") # Not provided
raw_igG   <- read.csv("") # Not provided
raw_MBC   <- read.csv("") # Not provided
raw_IC50  <- read.csv("") # Not provided
...
#######################################################
```{r}
immu_detail_individuals <- read.csv("immu_detail_individuals_reinf.csv") # not provided
```
```{r}
